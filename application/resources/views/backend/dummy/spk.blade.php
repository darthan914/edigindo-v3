...
@section('content')
	...
	<h1>Create Dummy SPK</h1>
	<div class="x_panel">
		<form class="form-horizontal form-label-left" action="{{ route('backend.dummy.createDummySpk') }}" method="post" enctype="multipart/form-data">
			...
			<div class="form-group">
				<label for="loop_production" class="control-label col-md-3 col-sm-3 col-xs-12">Rand Loop Production <span class="required">*</span>
				</label>
				<div class="col-md-9 col-sm-9 col-xs-12">
					<select id="loop_production" name="loop_production" class="form-control {{$errors->first('loop_production') != '' ? 'parsley-error' : ''}}">
						@for ($i = 0; $i < 5; $i++)
							<option value="{{ $i + 1 }}" @if(old('loop_production') == $i + 1) selected @endif>{{ $i + 1 }}</option>
						@endfor
					</select>
					<ul class="parsley-errors-list filled">
						<li class="parsley-required">{{ $errors->first('loop_production') }}</li>
					</ul>
				</div>
			</div>
