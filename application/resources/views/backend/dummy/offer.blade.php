...
@section('content')
	...
	<h1>Create Dummy Offer</h1>
	<div class="x_panel">
		<form class="form-horizontal form-label-left" action="{{ route('backend.dummy.createDummyOffer') }}" method="post" enctype="multipart/form-data">
			...
			<div class="form-group">
				<label for="loop_detail" class="control-label col-md-3 col-sm-3 col-xs-12">Rand Loop Detail <span class="required">*</span>
				</label>
				<div class="col-md-9 col-sm-9 col-xs-12">
					<select id="loop_detail" name="loop_detail" class="form-control {{$errors->first('loop_detail') != '' ? 'parsley-error' : ''}}">
						@for ($i = 0; $i < 5; $i++)
							<option value="{{ $i + 1 }}" @if(old('loop_detail') == $i + 1) selected @endif>{{ $i + 1 }}</option>
						@endfor
					</select>
					<ul class="parsley-errors-list filled">
						<li class="parsley-required">{{ $errors->first('loop_detail') }}</li>
					</ul>
				</div>
			</div>
