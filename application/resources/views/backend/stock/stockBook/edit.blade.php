...
@section('content')
	...
	<h1>Edit Stock Book</h1>
	<div class="x_panel">
	<form class="form-horizontal form-label-left" action="{{ route('backend.stock.updateStockBook', $index->id) }}" method="post" enctype="multipart/form-data">
		<div class="form-group">
			...
			<div class="col-md-9 col-sm-9 col-xs-12">
				<select id="stock_id" name="stock_id" class="form-control {{$errors->first('stock_id') != '' ? 'parsley-error' : ''}}" value="{{ old('stock_id') }}">
					...
					<option value="{{ $list->id }}" @if(old('stock_id', $index->stock_id) == $list->id) selected @endif>
						@if($list->photo)
		...
		<div class="form-group">
			...
			<div class="col-md-9 col-sm-9 col-xs-12">
				<input type="text" id="name_borrow" name="name_borrow" class="form-control {{$errors->first('name_borrow') != '' ? 'parsley-error' : ''}}" value="{{ old('name_borrow', $index->name_borrow) }}">
				<ul class="parsley-errors-list filled">
		...
		<div class="form-group">
			...
			<div class="col-md-9 col-sm-9 col-xs-12">
				<select id="need" name="need" class="form-control {{$errors->first('need') != '' ? 'parsley-error' : ''}}">
					...
					<option value="{{ $key }}" @if(old('need', $index->need) == $key) selected @endif>{{ $list }}</option>
					@endforeach
				</select>
				<ul class="parsley-errors-list filled">
		...
		<div class="form-group">
			<label for="quantity_borrow" class="control-label col-md-3 col-sm-3 col-xs-12">Jumlah Pinjam<span class="required">*</span>
			</label>
			<div class="col-md-9 col-sm-9 col-xs-12">
				<input type="number" id="quantity_borrow" name="quantity_borrow" class="form-control {{$errors->first('quantity_borrow') != '' ? 'parsley-error' : ''}}" value="{{ old('quantity_borrow', $index->quantity_borrow) }}">
				<ul class="parsley-errors-list filled">
		...
		<div class="form-group">
			<label for="date_borrow" class="control-label col-md-3 col-sm-3 col-xs-12">Tanggal Pnjam <span class="required">*</span>
			</label>
			<div class="col-md-9 col-sm-9 col-xs-12">
				<input type="date" id="date_borrow" name="date_borrow" class="form-control {{$errors->first('date_borrow') != '' ? 'parsley-error' : ''}}" value="{{ old('date_borrow', $index->date_borrow) }}">
				<ul class="parsley-errors-list filled">
		...
		<div class="form-group">
			...
			<div class="col-md-9 col-sm-9 col-xs-12">
				<input type="date" id="deadline_borrow" name="deadline_borrow" class="form-control {{$errors->first('deadline_borrow') != '' ? 'parsley-error' : ''}}" value="{{ old('deadline_borrow', $index->deadline_borrow) }}">
				<ul class="parsley-errors-list filled">
