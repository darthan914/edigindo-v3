...
@section('content')
	...
	<h1>Edit Stock</h1>
	<div class="x_panel">
	<form class="form-horizontal form-label-left" action="{{ route('backend.stock.update', ['id' => $index->id]) }}" method="post" enctype="multipart/form-data">
		...
		<div class="form-group">
			...
			<div class="col-md-9 col-sm-9 col-xs-12">
				<select id="stock_places_id" name="stock_places_id" class="form-control {{$errors->first('stock_places_id') != '' ? 'parsley-error' : ''}}">
					...
					<option value="{{ $list->id }}" @if(old('stock_places_id', $index->stock_places_id) == $list->id) selected @endif>{{ $list->name }}</option>
					@endforeach
				</select>
				<ul class="parsley-errors-list filled">
		...
		<div class="form-group">
			...
			<div class="col-md-9 col-sm-9 col-xs-12">
				<input type="number" id="quantity" name="quantity" class="form-control {{$errors->first('quantity') != '' ? 'parsley-error' : ''}}" value="{{ old('quantity', $index->quantity) }}">
				<ul class="parsley-errors-list filled">
