...
@section('content')
	...
	<div class="x_panel">
	<form class="form-horizontal form-label-left" action="{{ route('backend.designer.store') }}" method="post" enctype="multipart/form-data">
				...
				<select name="designer_id" id="designer_id" class="form-control {{$errors->first('designer_id') != '' ? 'parsley-error' : ''}}">
					...
					<optgroup label="Group {{$list->fullname}}">
						...
						@foreach($designer as $list2)
							...
							<option value="{!! $list2->id !!}" @if(old('designer_id') == $list2->id) selected @endif>{!! $list2->fullname !!}</option>
							@endif
						@endforeach
					</optgroup>
					@endforeach
					...
					@foreach($designer as $list)
						...
						<option value="{!! $list->id !!}" @if(old('designer_id') == $list->id) selected @endif>{!! $list->fullname !!}</option>
						{{-- @endif --}}
					@endforeach
				</select>
		...
		<div class="form-group">
			...
			<div class="col-md-9 col-sm-9 col-xs-12">
				<label class="checkbox-inline"><input type="checkbox" value="1" name="urgent" @if(old('urgent') == "1") checked @endif>Urgent</label>
				<ul class="parsley-errors-list filled">
