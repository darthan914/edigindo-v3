...
@section('content')
	...
	<h1>Edit Position</h1>
	<div class="x_panel">
	<form class="form-horizontal form-label-left" action="{{ route('backend.position.update', ['id' => $index->id]) }}" method="post" enctype="multipart/form-data">
		...
		<div class="form-group">
			...
			<div class="col-md-9 col-sm-9 col-xs-12">
				<select id="parent_id" name="parent_id" class="form-control select2 {{$errors->first('parent_id') != '' ? 'parsley-error' : ''}}">
					...
					<option value="{{ $list->id }}" @if(old('parent_id', $index->parent_id) == $list->id) selected @endif>{{ $list->name }}</option>
					@endforeach
				</select>
				<ul class="parsley-errors-list filled">
		...
		<div class="form-group">
			...
			<div class="col-md-9 col-sm-9 col-xs-12">
				<label class="checkbox-inline"><input type="checkbox" name="permission[]" class="group-master" value="configuration" @if(in_array('configuration', old('permission', explode(', ', $index->permission)))) checked @endif>Configuration</label>
				<ul class="parsley-errors-list filled">
		...
		<div class="form-group">
			...
			<div class="col-md-9 col-sm-9 col-xs-12">
				...
				<label class="checkbox-inline"><input type="checkbox" name="permission[]" class="group-developer" value="beta" @if(in_array('beta', old('permission', explode(', ', $index->permission)))) checked @endif>Beta Tester</label>
				<ul class="parsley-errors-list filled">
					<li class="parsley-required">{{ $errors->first('permission') }}</li>
				</ul>
			</div>
