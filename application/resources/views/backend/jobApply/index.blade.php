...
<script type="text/javascript">
	...
	$(function() {
		...
		var table = $('#datatable').DataTable({
			...
			serverSide: true,
			ajax: {
				url: "{{ route('backend.jobApply.datatables') }}",
				type: "POST",
				data: {
			...
			{data: 'position', sClass: 'nowrap-cell'},
		...
		$('#datatable').on('click', '.delete-jobApply', function(){
			$('#delete-jobApply input[name=id]').val($(this).data('id'));
		});
...
@can('delete-jobApply')
{{-- Delete SPK --}}
<div id="delete-jobApply" class="modal fade" role="dialog">
	<div class="modal-dialog">
		<div class="modal-content">
			<form class="form-horizontal form-label-left" action="{{ route('backend.jobApply.delete') }}" method="post" enctype="multipart/form-data">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal">&times;</button>
					<h4 class="modal-title">Delete Job Apply?</h4>
				</div>
...
<div class="x_panel" style="overflow: auto;">
	<div class="row">
		...
		<div class="col-md-6">
			<form method="post" id="action" action="{{ route('backend.jobApply.action') }}" class="form-inline text-right" onsubmit="return confirm('Are you sure to apply this selected?')">
