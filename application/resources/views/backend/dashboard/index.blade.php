	...
	$(function() {
				...
				var chartMonthly = new Chart(ctx, {
					...
					data: {
						...
						datasets: [{
							...
							data: data.chart_real_omset
						}, {
							label: 'Target Remaining',
							backgroundColor: "#395267",
							data: data.chart_sub_real_omset
						}]
		...
		$.ajax({
			...
			data: {
				...
				var ctx = document.getElementById("chart-campaign-expo");
				var chartMonthly = new Chart(ctx, {
					type: 'bar',
					data: {
						labels: data.label_month,
		...
		@foreach($campaign->campaign_details as $list)
		$.ajax({
			url: "{{ route('backend.home.ajaxChartCampaignDetail', $list->id) }}",
			type: "POST",
			data: {
				f_year: $('*[name=f_year]').val(),
			},
		}).done(function(data) {
				var ctx = document.getElementById("chart-campaign-{{ $list->id }}");
				var chartMonthly = new Chart(ctx, {
					type: 'bar',
					data: {
						labels: data.label_month,
...
@section('content')
	...
	<div class="row">
		...
		<div class="col-md-6 col-sm-6 col-xs-12">
			<div class="x_panel">
				<div class="x_title">
					<h2>Chart Campaign Expo (Million)</h2>
					<div class="clearfix"></div>
				</div>
				<div class="x_content">
					<canvas id="chart-campaign-expo"></canvas>
				</div>
			</div>
		</div>
	</div>
	...
	<div class="row">
		...
		<div class="col-md-4 col-sm-4 col-xs-12">
			<div class="x_panel">
				<div class="x_title">
					<h2>Target for {{ $list->name }} (Million)</h2>
					<div class="clearfix"></div>
				</div>
				<div class="x_content">
					<canvas id="chart-campaign-{{ $list->id }}"></canvas>
				</div>
			</div>
		</div>
		@endforeach
