...
@section('content')
	...
	<div id="create-detail" class="modal fade" role="dialog">
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				<form class="form-horizontal form-label-left" action="{{ route('backend.account.storeAccountPurchasingDetail') }}" method="post" enctype="multipart/form-data">
					...
					<div class="modal-footer">
						{{ csrf_field() }}
						<input type="hidden" name="account_purchasing_id" value="{{ $index->id }}">
	...
	<div id="edit-detail" class="modal fade" role="dialog">
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				<form class="form-horizontal form-label-left" action="{{ route('backend.account.updateAccountPurchasingDetail') }}" method="post" enctype="multipart/form-data">
					<div class="modal-header">
						<button type="button" class="close" data-dismiss="modal">&times;</button>
	...
	<div id="delete-detail" class="modal fade" role="dialog">
		<div class="modal-dialog">
			<div class="modal-content">
				<form class="form-horizontal form-label-left" action="{{ route('backend.account.deleteAccountPurchasingDetail') }}" method="post" enctype="multipart/form-data">
					<div class="modal-header">
						<button type="button" class="close" data-dismiss="modal">&times;</button>
	...
	<div class="x_panel">
	<form class="form-horizontal form-label-left" action="{{ route('backend.account.updateAccountPurchasing', $index->id) }}" method="post" enctype="multipart/form-data">
		...
		<div class="form-group">
			...
			<div class="col-md-9 col-sm-9 col-xs-12">
				<select id="supplier_id" name="supplier_id" class="form-control {{$errors->first('supplier_id') != '' ? 'parsley-error' : ''}} select2" data-placeholder="Select Purchasing" data-allow-clear="true">
					<option value=""></option>
					@foreach($supplier as $list)
					<option value="{{ $list->id }}" @if(old('supplier_id', $index->supplier_id) == $list->id) selected @endif>{{ $list->name }}</option>
					@endforeach
				</select>
				<ul class="parsley-errors-list filled">
		...
		<div class="form-group">
			<div class="col-md-6 col-sm-6 col-xs-12 col-md-offset-3">
				{{ csrf_field() }}
				<a class="btn btn-primary" href="{{ route('backend.account.accountPurchasing') }}">Back</a>
				<button type="submit" class="btn btn-success">Submit</button>
		...
		<table class="table" id="datatable">
			...
			<tfoot>
				...
				<th></th>
				<th style="text-align: right;">{{ number_format($index->account_purchasing_details()->sum(DB::raw('((price * qty) * (1 - (discount / 100))) * (1 + (ppn / 100))'))) }}</th>
				<th></th>
