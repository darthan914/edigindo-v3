@php
	$sum_price = $index->account_sales_details()->sum(DB::raw('price * qty'));
	$discount = $index->account_sales_details()->sum(DB::raw('(price * (discount / 100)) * qty'));
	$total = $sum_price - $discount;
	$sum_ppn = $index->account_sales_details()->sum(DB::raw('(price * qty) * (ppn /100)'));
@endphp
...
<title>{{ $index->invoice }} - {{ date('d/m/Y', strtotime($index->date)) }}</title>

<div>
	<table class="full collapse">
		<tr>
			<td rowspan="2" width="67%" style="text-align: center;"><b>FAKTUR PENUALAN INVOICE</b></td>
			<td width="33%" class="bordered"><span class="tab1">Tanggal</span> {{ date('d/m/Y', strtotime($index->date)) }}</td>
		</tr>
		<tr>
			<td class="bordered"><span class="tab1">No Faktur</span> {{ $index->invoice }}</td>
		</tr>
	</table>
	...
	<table class="full collapse">
		...
		@foreach($index->account_sales_details as $list)
		<tr class="bordered">
